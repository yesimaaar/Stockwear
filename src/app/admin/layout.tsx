...
import { LogOut, AlertTriangle, Bell, Loader2, RefreshCcw, Moon, Sun, PanelLeft, ShoppingCart, ArrowLeft } from "lucide-react";
...
import type { AdminSidebarProps, SidebarMode } from "@/components/domain/admin-sidebar";
...
const AdminSidebar = dynamic<AdminSidebarProps>(() => import("@/components/domain/admin-sidebar"), {
	ssr: false,
	loading: () => (
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
	...
	const formattedUpdatedAt = useMemo(() => {
		...
		return new Intl.DateTimeFormat("es-CO", { hour: "2-digit", minute: "2-digit" }).format(lastUpdated);
	}, [lastUpdated]);

	const trimmedSearchValue = searchTerm.trim();
	const selectedModule = useMemo(
		() => SEARCHABLE_MODULES.find((m) => m.href === searchModule) ?? SEARCHABLE_MODULES[0] ?? ADMIN_NAV_ITEMS[0],
		[searchModule]
	);
	...
	const handleSearchSubmit = useCallback(
		(event: FormEvent<HTMLFormElement>) => {
			...
			if (!canSubmitSearch || !searchTargetHref) return;
					...
					<div className="flex h-16 w-full items-center gap-3 px-4 sm:px-6 lg:px-10">
						...
						<div ref={searchWrapperRef} className="relative flex flex-1 items-center">
							<form onSubmit={handleSearchSubmit} className="relative w-full">
								<HeaderSearchBar
									value={searchTerm}
									onChange={next => { setSearchTerm(next); if (canShowSearchModules) setSearchPanelOpen(true); }}
