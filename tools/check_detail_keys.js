...
async function main() {
  console.log("Checking ventasDetalle structure...");
  
  // Insert a dummy row to test column names? No, might violate constraints.
  // Better to fetch ANY row.
  ...
  if (error) {
    ...
    if (data.length > 0) {
      console.log("Keys found in ventasDetalle:", Object.keys(data[0]));
    } else {
      console.log("No rows in ventasDetalle found.");
    }
  }
}
